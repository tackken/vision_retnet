...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, msk):
        ...
        weights_before = attention_weight / math.sqrt(self.dim_heads)
        weights_after = attention_weight / math.sqrt(self.dim_heads)
...
class MLPHead(nn.Module):
    def __init__(self, dim, out_dim, dropout):
        super().__init__()
        self.linear_layer = nn.Linear(dim, out_dim, bias=True)
        init.xavier_uniform_(self.linear_layer.weight) # WEIGHT
        init.zeros_(self.linear_layer.bias) # BIAS
